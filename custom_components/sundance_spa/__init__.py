...
CHECKS_BEFORE_RETRY   = 2   # wie HyperActiveJ-Referenz
...
class SpaClient:
    ...
    async def _handle_temp_feedback(self, status: dict) -> None:
        """Feedback-Schleife analog HyperActiveJ parse_C4status_update."""
        ...
        if abs(current - self._target_temp) < 0.3:
            ...
            self._temp_done.set()
            _LOGGER.info(
                "Soll-Temperatur erreicht: %.1f °C | kanal=0x%02X sent=%d attempts=%d",
                current,
                self._assigned_channel or 0,
                self._cc_sent,
        ...
        # Wenn seit letztem Versuch nichts gesendet wurde: CTS-Problem loggen
        if self._command_attempts > 0 and self._cc_sent == 0:
            _LOGGER.warning(
                "Temp-CC noch NIE gesendet (queued=%d cts_own=%d kanal=0x%02X) "
                "– warte auf CTS. discovered=%s",
                self._cc_queued,
                self._cts_own,
                self._assigned_channel or 0,
                [f"0x{c:02X}" for c in self._discovered_channels],
