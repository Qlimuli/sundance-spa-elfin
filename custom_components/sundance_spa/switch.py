...
class SpaSwitch_(CoordinatorEntity, SwitchEntity):
    ...
    def extra_state_attributes(self) -> dict:
        ...
        if self._sw.key == "blower":
            return {
                "panel_cc": f"{BLOWER_CC_BTN}/{BLOWER_CC_B6}",
                "note": "Kein RS485-Status – optimistischer Schalter",
            }
        return {}

    async def _send_blower_toggle(self) -> None:
        """Blubber: verschlüsseltes Panel-CC (53/217)."""
        await self.coordinator.client.send_blower_toggle()
        await self.coordinator.client.wait_status(n=4, timeout=5.0)
